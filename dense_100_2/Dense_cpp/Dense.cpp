...
uint get_main_input_signal_no() {return 302;}

uint get_total_signal_no() {return 1709;}

uint get_number_of_components() {return 7;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 503;}
...
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 302;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
ctx->componentMemory[coffset].subcomponents = new uint[2]{0};
}
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + (((2 * Fr_toInt(&lvar[3])) + (1 * Fr_toInt(&lvar[2]))) + 102)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
matMul_2_run(mySubcomponents[cmp_index_ref],ctx);
...
// load src
Fr_add(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[2])) + 0)]].signalStart + 0],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 302)]); // line circom 21
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
