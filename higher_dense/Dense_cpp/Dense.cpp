...
uint get_main_input_signal_no() {return 120;}

uint get_total_signal_no() {return 851;}

uint get_number_of_components() {return 31;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 221;}
...
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 120;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[2])) + 0);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + (((1 * Fr_toInt(&lvar[3])) + 0) + 11)];
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + (((10 * Fr_toInt(&lvar[3])) + (1 * Fr_toInt(&lvar[2]))) + 20)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
matMul_2_run(mySubcomponents[cmp_index_ref],ctx);
...
// load src
Fr_add(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[2])) + 0)]].signalStart + 0],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 120)]); // line circom 21
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
