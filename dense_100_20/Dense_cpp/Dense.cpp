...
NULL,
NULL };
uint get_main_input_signal_start() {return 21;}

uint get_main_input_signal_no() {return 2120;}

uint get_total_signal_no() {return 16181;}

uint get_number_of_components() {return 61;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 4121;}
...
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2120;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
uint aux_create = 0;
int aux_cmp_num = 0+ctx_index+1;
uint csoffset = mySignalStart+2140;
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[3])) + 20)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + (((20 * Fr_toInt(&lvar[3])) + (1 * Fr_toInt(&lvar[2]))) + 120)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
matMul_2_run(mySubcomponents[cmp_index_ref],ctx);
...
// load src
Fr_add(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[2])) + 0)]].signalStart + 0],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 2120)]); // line circom 21
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
