...
NULL,
NULL };
uint get_main_input_signal_start() {return 76;}

uint get_main_input_signal_no() {return 7675;}

uint get_total_signal_no() {return 60401;}

uint get_number_of_components() {return 226;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 15176;}
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + ((0 + (1 * Fr_toInt(&lvar[3]))) + 100)],&signalValues[mySignalStart + ((0 + (1 * Fr_toInt(&lvar[3]))) + 200)]); // line circom 11
// end load src
...
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 101)];
// load src
Fr_sub(&expaux[1],&lvar[2],&circuitConstants[0]); // line circom 15
Fr_add(&expaux[0],&signalValues[mySignalStart + ((1 * Fr_toInt(&expaux[1])) + 101)],&signalValues[mySignalStart + ((0 + (1 * Fr_toInt(&lvar[4]))) + 1)]); // line circom 15
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
}
...
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + ((0 + (1 * Fr_toInt(&lvar[5]))) + 100)];
// load src
// end load src
...
uint cmp_index_ref = 0;
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + ((0 + (1 * Fr_toInt(&lvar[5]))) + 200)];
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + (((1 * Fr_toInt(&lvar[5])) + 0) + 101)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
matElemMul_0_run(mySubcomponents[cmp_index_ref],ctx);
...
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 7675;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
ctx->componentMemory[coffset].subcomponents = new uint[75]{0};
}
...
uint aux_create = 0;
int aux_cmp_num = 0+ctx_index+1;
uint csoffset = mySignalStart+7750;
uint aux_dimensions[1] = {75};
for (uint i = 0; i < 75; i++) {
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[3])) + 75)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
...
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[2])) + 0);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + (((1 * Fr_toInt(&lvar[3])) + 0) + 101)];
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + (((75 * Fr_toInt(&lvar[3])) + (1 * Fr_toInt(&lvar[2]))) + 175)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
matMul_2_run(mySubcomponents[cmp_index_ref],ctx);
...
// load src
Fr_add(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[2])) + 0)]].signalStart + 0],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 7675)]); // line circom 21
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
